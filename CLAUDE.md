...
`app.js`, `styles.css` and `index.html` at the repo root are **mirrored output, not source**. The Mac's copies in
`draft-site/` are authoritative and the publisher force-pushes over the repo. If you edit them here they will be
**silently reverted at the next publish** — and Sean publishes after every change, so that is usually within the
hour. `sync_tools.py` does not pull them back.

Front-end changes therefore have to be made on the Mac. Either hand Sean a patch to apply, or ask him to run the
edit in a session that has the Mac. (If the next chat wants the front end round-tripped too, the clean way is to
add `app.js`/`styles.css`/`index.html` to `FILES` in `sync_tools.py` under `src/` paths and have `publish.sync()`
copy from there — deliberately **not** done yet, because it would double every site file in the repo.)
...
| `build_history.py [years…]` | `hist/mlb-YYYY.js` (+ days) | re-runs `build_data.py` season by season for 2015-2025. `build_history.py index` rebuilds `hist/index.js` (the search index). `spring 2026 2025` / `post 2025 2024` build those game types as their own datasets. Rows are player × handedness × venue — no date dimension, so past seasons have splits but not date windows |
...
| `build_fantasy.py [years…]` | `fantasy.js`, `hist/fantasy-YYYY.js` | official counting stats + per-game pitching logs + Savant expected stats. Points are computed in the browser from the chosen scoring preset, so an ESPN setting change needs no rebuild |
...
| `tools/sync_tools.py` | the round trip described in §1 |
...
* **Publish after every change.** `python3 publish_github.py`. He checks on his phone against the hosted copy; an
  unpublished change is invisible to him.
